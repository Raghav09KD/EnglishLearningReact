...
const Home = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth < 768); // Check if the screen width is less than 768px (md breakpoint)
    };
  ...
  return (
    <div className="bg-gradient-to-r from-gray-50 to-gray-100">
      ...
      <section
        className={`relative flex items-center justify-center px-8 pt-28 pb-32 min-h-[70vh] overflow-hidden ${!isMobile ? "bg-cover bg-no-repeat" : ""}`}
        style={{
          backgroundImage: !isMobile ? `url(${bgImage})` : "none", // Apply background image only on larger screens
